                      ...
                      @foreach ($dataType->browseRows as $row)
                        ...
                        <td>
                          ...
                          @elseif($row->type == 'image')
                            <img
                              src="@if (!filter_var($data->{$row->field}, FILTER_VALIDATE_URL)) {{ fromDLHost($data->{$row->field}) }}@else{{ $data->{$row->field} }} @endif"
                              style="width:100px">
                          ...
                          @elseif($row->type == 'multiple_images')
                            ...
                            @if ($images)
                              ...
                              @foreach ($images as $image)
                                <img
                                  src="@if (!filter_var($image, FILTER_VALIDATE_URL)) {{ fromDLHost($image) }}@else{{ $image }} @endif"
                                  style="width:50px">
                              @endforeach
                            @endif
                            ...
                            @if ($files)
                              @if (property_exists($row->details, 'show_as_images') && $row->details->show_as_images)
                                @foreach (array_slice($files, 0, 3) as $file)
                                  <img
                                    src="@if (!filter_var($file, FILTER_VALIDATE_URL)) {{ fromDLHost($file) }}@else{{ $file }} @endif"
                                    style="width:50px">
                                @endforeach
                              @else
                                <ul>
...
@section('javascript')
  ...
  <script>
    ...
    $('td').on('click', '.delete', function(e) {
      $('#delete_form')[0].action = '{{ route('voyager.' . $dataType->slug . '.destroy', '__id') }}'.replace('__id', $(
        this).data('id'));
